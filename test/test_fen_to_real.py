...
# Using your batch Blender script
...
TARGET_SIZE = 2048  # high resolution as requested
TILES_PER_SIDE = 8
BORDER_CUT = 0      # same as in new_8X8_to_full.py
DEVICE = "cuda" if torch.cuda.is_available() else "cpu"

# =========================
# 2) Model (ResnetGenerator)
# =========================
...
def find_blender_exe(project_root):
    # Search for Blender locally or in PATH
...
def run_blender_generation(blender_exe, fen):
    """
    Run Blender to generate a synthetic image.
    Important fix: --resolution 2048 and cleanup of previous files.
    """
    print(f"[INFO] 1. Rendering synthetic image in Blender (2048x2048)...")

    BLENDER_OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    # Use 1_overhead.png as default output
    expected_output = BLENDER_OUTPUT_DIR / "1_overhead.png"

    # Remove old file so Blender won't skip rendering
    ...
    cmd = [
        ...
        "--",
        "--fen", fen,
        "--resolution", str(TARGET_SIZE),  # <--- critical fix: 2048
...
def process_image_full_logic(netG, img_path, device):
    """
    Exact logic from new_8X8_to_full.py:
    Load -> Resize (2048) -> Split to 8x8 -> Inference -> Stitch
    """
    ...
    # Border crop (if enabled)
    if BORDER_CUT > 0:
        img = img.crop((BORDER_CUT, BORDER_CUT, w - BORDER_CUT, h - BORDER_CUT))

    # High-quality resize to 2048
    img = img.resize((TARGET_SIZE, TARGET_SIZE), Image.BICUBIC)

    # Prepare canvas for final image
