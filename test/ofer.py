...
def main():
    ...
    # --- 1. חישוב נתיבים דינמי (הופך את הסקריפט לכללי) ---
    # מזהה איפה הקובץ הנוכחי נמצא
    current_script_path = Path(__file__).resolve()
    # מניח ש-ofer.py נמצא בתוך תיקיית test, אז עולים שתי רמות למעלה לשורש הפרויקט
    project_root = current_script_path.parent.parent 
    
    # נתיב לבלנדר בתיקיית הבית
    default_blender = Path.home() / "blender-3.6.5-linux-x64/blender"

    # --- הגדרת ארגומנטים ---
    p.add_argument("--blender", default=str(default_blender))
    
    # שינוי חשוב: הנתיבים מצביעים לתוך תיקיית blender/
    ...
    # ====================
    zoom_cmd = [
        sys.executable,  # שימוש בפייתון הנוכחי (חשוב!)
        str(Path(args.zoom_script)),
        "--in-root", str(render_dir),
        "--out-root", str(zoomed_dir),
        "--zoom", str(args.zoom),
        "--overwrite" # מכריחים דריסה כדי לוודא שזה תמיד רץ
    ]
    ...
    try:
        # capture_output=True עוזר לנו לתפוס שגיאות במקרה של כישלון
    ...
    model_cmd = [
        sys.executable, # גם כאן משתמשים בפייתון הנוכחי
