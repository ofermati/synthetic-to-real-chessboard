...
def set_crop_to_object(scene, cam_obj, target_obj, pad=0.02):
    """Crop render to the screen-space bounding box of target_obj.

    This keeps the original camera perspective (NO warp) and removes the gray
    background by rendering only the board region.
    """
    depsgraph = bpy.context.evaluated_depsgraph_get()
    obj_eval = target_obj.evaluated_get(depsgraph)

    # bound_box corners are in local space -> transform to world space
    bbox_world = [obj_eval.matrix_world @ Vector(corner) for corner in obj_eval.bound_box]
    coords = [world_to_camera_view(scene, cam_obj, v) for v in bbox_world]

    min_x = max(0.0, min(c.x for c in coords) - pad)
    max_x = min(1.0, max(c.x for c in coords) + pad)
    min_y = max(0.0, min(c.y for c in coords) - pad)
    max_y = min(1.0, max(c.y for c in coords) + pad)
...
def reset_pieces_to_start(starting_pieces):
    """Reset all pieces to their original detected starting positions."""
    for piece_name, info in starting_pieces.items():
        obj = bpy.data.objects.get(piece_name)
        if obj:
...
def render_all_views(board_info, view='black', out_dir=None, tag=''):
    ...
    if out_dir is None:
        out_dir = OUT_DIR
    # ensure folder exists (OUT_DIR may be relative to .blend)
    ...
    scene = bpy.context.scene
    board_plane = bpy.data.objects.get("Black & white")
    ...
    for location, name, point_at_center in views:
        ...
        # white / black view rotation
        cam.rotation_euler.z += z_rotation_offset

        # --- crop to board ---
        if board_plane is not None:
            set_crop_to_object(scene, cam, board_plane, pad=0.01)

        # --- render ---
        fname = f"{tag}_{name}.png" if tag else f"{name}.png"
        out_path = os.path.join(out_abs, fname)
...
def main():
    ...
    parser.add_argument('--fens_dir', type=str, default=None,
                        help='Path to a folder containing game*.csv files (with columns: from_frame,to_frame,fen). If set, renders all positions in all CSVs.')
    parser.add_argument('--max_positions', type=int, default=None,
                        help='Optional cap on number of positions per CSV (for quick tests).')
    parser.add_argument('--every', type=int, default=1,
                        help='Render every Nth position in each CSV (default 1 = render all).')
    ...
    # Detect starting positions (once)
    starting_pieces = detect_starting_positions(board_info)

    # =========================
    # SINGLE FEN or BATCH CSVs
    # =========================
    if args.fens_dir:
        fens_dir = Path(args.fens_dir)
        csv_files = sorted([p for p in fens_dir.iterdir() if p.suffix.lower() == ".csv"])
        if not csv_files:
            raise FileNotFoundError(f"No .csv files found under: {fens_dir}")

        print("\n" + "="*70)
        print(f"BATCH MODE: rendering {len(csv_files)} CSV files from: {fens_dir}")
        print("="*70)
        ...
        for csv_path in csv_files:
            ...
            for i, row in enumerate(rows):
                if args.every > 1 and (i % args.every) != 0:
                    continue
                if args.max_positions is not None and rendered >= args.max_positions:
                    break
                ...
                frame = (row.get("from_frame") or row.get("to_frame") or str(i)).strip()
                out_dir = f"{OUT_DIR}/{game_name}/frame_{frame}"

                print(f"\n[GAME {game_name}] position #{i} frame={frame}")
                reset_pieces_to_start(starting_pieces)
                apply_fen(fen, starting_pieces, board_info)
                render_all_views(board_info, view=args.view, out_dir=out_dir)

                rendered += 1

            print(f"Done {game_name}: rendered {rendered} positions")

    else:
        # Single FEN (debug / quick test)
        reset_pieces_to_start(starting_pieces)
